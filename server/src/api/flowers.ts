...
export async function searchFlowers(query: string): Promise<WeddingOption[]> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[searchFlowers] API error:", response.status, errorBody);
      return mockData.flowers;
    }

    const data = await response.json();
    console.log("[searchFlowers] Results count:", data.places?.length ?? 0);
    if (!data.places?.length) return mockData.flowers;
    ...
    return data.places.map((place: any, i: number) => {
      const photoUrl = place.photos?.[0]
        ...
        : `https://picsum.photos/seed/flowers-${i}/400/300`;
      ...
      return {
        id: `flowers-${place.id}`,
        name: place.displayName?.text ?? `Florist ${i + 1}`,
        description: place.editorialSummary?.text ?? place.formattedAddress ?? "Wedding florist & decoration",
        price: 0,
        imageUrl: photoUrl,
        details,
    ...
    console.error("Failed to fetch flowers from Google Places:", error);
    return mockData.flowers;
  }
}
